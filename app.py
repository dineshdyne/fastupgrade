...
check_points=st.slider("Number of check points",min_value=2,max_value=10,step=1)
    
if col_b:  
    l=[] 
    for i in range(check_points):
        col1,col2=st.columns([3,3])
        col1=st.number_input(" Required Amount" ,key=f"col1_{i}",min_value=5,step=5)
        col2=st.number_input(" % increment in rate of gain" ,key=f"col2_{i}",min_value=5,max_value=2000,step=5)
        l.append((col1,col2))
